...
interface MacroIndicator {
  ...
  indicator: string;
  name: string;
  category: 'Trend' | 'Liquidity' | 'Risk' | 'Economy' | 'Breadth';
  value: number;
...
interface RegimeSummary {
  regime: 'Strong Bull' | 'Bull' | 'Neutral' | 'Bear' | 'Crash';
  score: number;
...
const MacroCard = ({ item, onClick }) => {
  ...
  const statusColors = {
    Bull: 'text-[#FF4B4B] bg-[#FF4B4B]/10 border-[#FF4B4B]/30',
    Neutral: 'text-yellow-500 bg-yellow-500/10 border-yellow-500/30',
    Bear: 'text-[#3B82F6] bg-[#3B82F6]/10 border-[#3B82F6]/30'
  };
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <h3 className="text-[13px] md:text-[14px] font-extrabold text-slate-500 dark:text-slate-400 group-hover:text-slate-800 dark:group-hover:text-slate-200 transition-colors truncate pr-2">
          {item.name}
        </h3>
        <span className={`text-[10px] font-black px-2 py-0.5 rounded border ${statusColors[item.status]} shrink-0`}>
          {item.status}
        </span>
      </div>
  ...
  return (
    <div className="mb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map(item => (
          <MacroCard key={item.id} item={item} onClick={onCardClick} />
        ))}
      </div>
    </div>
  );
...
const MacroChartModal = ({ indicator, onClose }) => {
  ...
  const mockHistory = useMemo(() => {
    ...
    for(let i = days; i >= 0; i--) {
       ...
       val = val * (1 + (Math.random() * 0.04 - 0.02)); // Random walk
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 w-full max-w-4xl rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center p-5 md:p-6 border-b border-slate-100 dark:border-slate-800/80">
          <div>
            <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-1">{indicator.name}</h3>
            <p className="text-[13px] font-extrabold text-slate-500">{indicator.category} Indicator</p>
          </div>
        ...
        <div className="p-6 md:p-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
             <div>
                ...
                <span className={`text-[14px] font-black px-2.5 py-1 rounded-lg border ${indicator.change_percent >= 0 ? 'text-[#FF4B4B] bg-[#FF4B4B]/10 border-[#FF4B4B]/30' : 'text-[#3B82F6] bg-[#3B82F6]/10 border-[#3B82F6]/30'}`}>
...
const MacroPage = () => {
  ...
  return (
    <div className="animate-in fade-in duration-300 w-full">
      ...
      {selectedMacro && (
        <MacroChartModal indicator={selectedMacro} onClose={() => setSelectedMacro(null)} />
      )}
    </div>
  );
};
