...
const MICRO_STYLES = `
  ...
  @keyframes qsPresetPop { 0% { transform: scale(0.94); } 55% { transform: scale(1.05); } 100% { transform: scale(1); } }
  ...
  @keyframes qsVertexPop { 0% { r: 4; } 35% { r: 8; } 100% { r: 4; } }
  ...
  .qs-card { transition: transform 0.18s cubic-bezier(0.22, 1, 0.36, 1), box-shadow 0.18s ease, border-color 0.18s ease; }
...
function formatFinancial(v) { return v == null || isNaN(v) ? "N/A" : Number(v).toLocaleString(undefined, { maximumFractionDigits: 0 }); }
...
function MiniSnowflake({ row, size = 34 }) {
  ...
  const shapePoints = AXES.map((ax, i) => {
    const v = row[ax.key];
    const frac = (v == null || isNaN(v)) ? 0.08 : v / 100;
    const { x, y } = pt(i, frac);
    return `${x.toFixed(1)},${y.toFixed(1)}`;
  }).join(' ');
  const bgPoints = AXES.map((_, i) => pt(i, 1)).map(p => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ');
  const tooltipText = AXES.map(ax => {
    const v = row[ax.key];
    return `${ax.label}: ${v != null && !isNaN(v) ? v.toFixed(0) : 'N/A'}`;
  }).join('\n');
          ...
          return (
            <div key={ax.key} className="flex items-center gap-2" title={ax.desc}>
              ...
              <div className="flex gap-[3px] flex-1">
                {PRESET_VALUES.map(v => (
                  <button key={v} onClick={() => onAxisChange(ax.key, current === v ? null : v)} className={`qs-preset-chip flex-1 text-[9.5px] lg:text-[10px] font-black py-1 px-0.5 rounded-md border cursor-pointer whitespace-nowrap ${current === v ? 'text-white border-transparent' : 'text-slate-500 dark:text-slate-400 border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-[#151924] hover:border-slate-400 dark:hover:border-slate-500'}`} style={current === v ? { backgroundColor: ax.color } : {}}>≥{v}</button>
                ))}
              </div>
            </div>
          );
...
function ScreenerReportModal({ selectedStock, reportLoading, onClose }) {
  ...
  const gates = AXES.map(ax => {
    ...
    return { key: ax.key, label: ax.short, desc: ax.desc, score: score ?? null, pass: score != null && score >= 70 };
  });
          ...
          {reportLoading || selectedStock.isLoading ? (
            ...
            <>
              <div className="mb-8 md:mb-10">
                <h2 className="text-3xl md:text-5xl font-black text-slate-900 dark:text-white mb-2 md:mb-4 leading-tight tracking-tight">{selectedStock.name}</h2>
                <h1 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight flex items-baseline">{formatWon(selectedStock.current_price)} 원 <span className={`text-[16px] md:text-[24px] ml-2 md:ml-3 ${(selectedStock.ret_1m || 0) > 0 ? 'text-[#FF4B4B]' : 'text-[#3B82F6]'}`}>{(selectedStock.ret_1m || 0) > 0 ? '+' : ''}{formatPct(selectedStock.ret_1m || 0)} (1M)</span></h1>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                <div className="p-6 md:p-8 bg-slate-50 dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                  ...
                  <div><p className="text-[12px] md:text-[13px] font-extrabold text-slate-500 mb-1">통과 조건</p><p className="text-4xl md:text-5xl font-black text-[#00B464]">{animatedPassCount.toFixed(0)} <span className="text-2xl text-slate-400">/ 6</span></p></div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                  {gates.map(gate => (
                    <div key={gate.key} className={`p-4 rounded-2xl border flex flex-col justify-between h-28 ${gate.pass ? 'qs-gate-pass' : 'bg-slate-50 dark:bg-[#1E2329] border-slate-200 dark:border-slate-800'}`}>
                      <div className="flex justify-between items-center mb-2"><span className={`font-black text-[14px] md:text-[15px] ${gate.pass ? 'text-[#00B464]' : 'text-slate-400'}`}>{gate.label}</span><span className="text-[12px]">{gate.pass ? '✔️' : '❌'}</span></div>
                  ...
                  {selectedStock.chart_data && selectedStock.chart_data.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={selectedStock.chart_data} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                        ...
                        <YAxis domain={['auto', 'auto']} tick={{fill: '#94A3B8', fontSize: 11, fontWeight: '800'}} tickLine={false} axisLine={false} tickFormatter={(value) => value != null ? Math.round(value).toLocaleString() : ''} />
                        <Tooltip contentStyle={{backgroundColor: '#0F172A', borderColor: '#334155', borderRadius: '12px', color: 'white', fontWeight: '900'}} labelStyle={{color: '#94A3B8', marginBottom: '4px'}} formatter={(value, name) => [value != null ? Math.round(value).toLocaleString() : '', name]} />
  ...
  return (
    <div className="relative w-full min-w-0 pb-20 font-['Nunito',_ui-rounded,_-apple-system,_system-ui,_sans-serif]">
      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between md:items-center gap-3">
        <div>
          <h2 className="text-2xl md:text-[28px] font-black text-slate-900 dark:text-white flex items-center gap-3 tracking-tight">🔎 스크리너 <span className="text-[13px] font-black text-slate-400 tracking-normal">Minervini Trend Template</span></h2>
          <p className="text-[13px] font-bold text-slate-500 mt-1">추세·모멘텀 6조건으로 정배열 구간의 종목을 찾아보세요.</p>
        </div>
      </div>
      <div className="flex flex-col md:flex-row gap-3 mb-4">
        <div className="relative flex-1"><Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" /><input value={search} onChange={e => { setSearch(e.target.value); setVisibleCount(PAGE_SIZE); }} placeholder="종목명 또는 코드로 검색" className="w-full pl-11 pr-4 py-3 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-xl text-[14px] font-bold text-slate-900 dark:text-white placeholder:text-slate-500 placeholder:font-semibold focus:outline-none focus:border-blue-400 dark:focus:border-slate-600 transition-colors" /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 min-w-0">
        ...
        <div className="min-w-0">
          {(screenerData || []).length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-2xl"><div className="w-14 h-14 rounded-full bg-slate-100 dark:bg-[#151924] flex items-center justify-center mb-4"><Sparkles className="text-slate-400" size={24} /></div><p className="text-[16px] font-black text-slate-900 dark:text-white mb-1">스크리너 데이터가 없습니다</p><p className="text-[13px] font-bold text-slate-500">다음 배치(Cron) 실행 후 다시 확인해 주세요.</p></div>
          ) : !hasAnyFilter ? (
            <div className="flex flex-col items-center justify-center py-24 text-center bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-2xl"><div className="w-14 h-14 rounded-full bg-slate-100 dark:bg-[#151924] flex items-center justify-center mb-4"><Sparkles className="text-slate-400" size={24} /></div><p className="text-[16px] font-black text-slate-900 dark:text-white mb-1">조건을 하나 이상 설정해보세요</p><p className="text-[13px] font-bold text-slate-500">축 프리셋을 누르거나, 위의 전략 버튼 또는 섹터 필터로 바로 시작할 수 있어요.</p></div>
          ) : (
            <>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <select value={sortKey || ''} onChange={e => { setSortKey(e.target.value || null); setVisibleCount(PAGE_SIZE); }} className="qs-select pl-3 pr-9 py-2 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-lg text-[12.5px] font-bold text-slate-700 dark:text-slate-300 focus:outline-none focus:border-blue-400 dark:focus:border-slate-600 cursor-pointer">
                      <option value="">기본 순서</option>
                      {SORT_OPTIONS.map(opt => <option key={opt.key} value={opt.key}>{opt.label} 순</option>)}
                    </select>
                  </div>
                  <button onClick={() => setSortDir(d => d === 'desc' ? 'asc' : 'desc')} disabled={!sortKey} title={sortDir === 'desc' ? '내림차순' : '오름차순'} className={`p-2 rounded-lg border transition-colors ${sortKey ? 'bg-white dark:bg-[#111827] border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:border-blue-400 dark:hover:border-slate-500 cursor-pointer' : 'bg-slate-50 dark:bg-[#0B1120] border-slate-100 dark:border-slate-800/60 text-slate-300 dark:text-slate-700 cursor-not-allowed'}`}>
                    {!sortKey ? <ArrowUpDown size={14} /> : sortDir === 'desc' ? <ArrowDown size={14} /> : <ArrowUp size={14} />}
                  </button>
                </div>
              </div>
              {results.length === 0 ? (
                <div className="p-10 text-center text-slate-500 font-extrabold bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 rounded-2xl">조건에 맞는 종목이 없습니다.</div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {results.map((r) => <ScreenerCard key={r.symbol} r={r} onNameClick={handleNameClick} />)}
                </div>
              )}
              {hasMore && (
                <div className="flex justify-center mt-4">
                  <button onClick={() => setVisibleCount(c => c + PAGE_SIZE)} className="px-5 py-2.5 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-xl text-[13px] font-black text-slate-700 dark:text-slate-300 hover:border-blue-400 dark:hover:border-slate-500 cursor-pointer shadow-sm">더 보기 ({totalCount - visibleCount}개 남음)</button>
                </div>
              )}
            </>
          )}
      ...
      <ScreenerReportModal selectedStock={selectedStock} reportLoading={reportLoading} onClose={() => setSelectedStock(null)} />
    </div>
  );
}
