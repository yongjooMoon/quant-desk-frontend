...
export const fetchApi = async (endpoint, options = {}) => {
  ...
  try {
    ...
    // 필요 시 여기에 글로벌 에러 처리 로직 추가 (예: 토스트 알림 띄우기)
    throw error; 
  }
};
```
...
### STEP 2. `NewsDesk.jsx` 리팩토링 (공통 API 적용)

방금 주신 "진짜 원본 코드(디자인 완벽본)"에서 **지저분한 TTL 코드를 싹 날리고**, 상단에 공통 `fetchApi`를 임포트하여 통신 부분을 깔끔하게 한 줄로 줄였습니다. (나머지 로직과 애니메이션, 디자인은 단 1픽셀도 건드리지 않았습니다!)

아래 코드로 `quant-frontend/src/pages/NewsDesk.jsx`를 덮어씌워 주세요.

```react:NewsDesk (공통 API 적용):quant-frontend/src/pages/NewsDesk.jsx
...
export default function NewsDesk() {
  ...
  // 🌟 [변경] 공통 모듈인 fetchApi를 사용하여 통신 로직 간소화 (캐싱 날림)
  ...
  const getCategoryStyle = (category) => {
    const c = category || "";
    if (c.includes("지수")) return "text-[#60A5FA] bg-[#60A5FA]/10 border border-[#60A5FA]/20";
    if (c.includes("산업")) return "text-[#34D399] bg-[#34D399]/10 border border-[#34D399]/20";
    if (c.includes("에너지")) return "text-[#FBBF24] bg-[#FBBF24]/10 border border-[#FBBF24]/20";
    if (c.includes("금리")) return "text-[#818CF8] bg-[#818CF8]/10 border border-[#818CF8]/20";
    if (c.includes("글로벌")) return "text-[#F87171] bg-[#F87171]/10 border border-[#F87171]/20";
    return "text-[#A78BFA] bg-[#A78BFA]/10 border border-[#A78BFA]/20";
  };
                ...
                >
                  {todayMajorNews.map((item) => (
                    ...
                    >
                      ...
                      {showSectorOnCard && (
                        <div className="mt-4">
                          <span className={`text-[12px] font-extrabold px-3 py-1.5 rounded-lg border shadow-sm ${getCategoryStyle(getShortCategoryName(item.sector_asset))}`}>
                            #{getShortCategoryName(item.sector_asset)}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              {filteredList.length > 0 ? filteredList.slice(0, 50).map((item) => (
                <div key={item.id} onClick={() => setSelectedNews(item)} className="p-4 md:p-5 border-b border-slate-100 dark:border-slate-800/80 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/40 cursor-pointer transition-colors flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    {showSectorOnCard && (
                      <span className={`text-[11.5px] font-black px-2 py-1 rounded shrink-0 shadow-sm ${getCategoryStyle(getShortCategoryName(item.sector_asset))}`}>
                        {getShortCategoryName(item.sector_asset)}
                      </span>
                    )}
